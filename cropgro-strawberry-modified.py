...
def run_example_simulation():
    ...
    cultivar_params = {
        ...
        'sla': 0.02,        # Specific leaf area (m^2/g)
...
if __name__ == "__main__":
    ...
    print("\n=== Strawberry Growth Simulation Results ===")
    ...
    print(f"Final leaf area index: {results['leaf_area_index'].iloc[-1]:.2f} m^2/m^2")
    print(f"Final phenological stage: {results['stage'].iloc[-1]}")
    print(f"Total days simulated: {len(results)}")
    
    output_file = 'strawberry_simulation_results.png'
    fig.savefig(output_file, dpi=150, bbox_inches='tight')
    print(f"\nChart saved to: {output_file}")
    print(f"Open this file in your image viewer to see the results.")
    
    csv_file = 'strawberry_simulation_data.csv'
    results.to_csv(csv_file, index=False)
    print(f"Data saved to: {csv_file}")
    
    try:
        print(f"\nChart window should appear. Close it to exit...")
        plt.show(block=True)
    except Exception as e:
        print(f"\nNote: Chart window could not open due to: {str(e)}")
        print(f"Please view the saved image file: {output_file}")