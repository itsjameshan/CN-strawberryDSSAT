    ...
    for line in lines[start + 1:]:
        ...
        rec = {
            ...
            "tmax": (float(parts[indices["TMAX"]]) 
                    if "TMAX" in indices and len(parts) > indices["TMAX"] 
                    else 25.0),
            "tmin": (float(parts[indices["TMIN"]]) 
                    if "TMIN" in indices and len(parts) > indices["TMIN"] 
                    else 15.0),
            "solar_radiation": (float(parts[indices["SRAD"]]) 
                               if "SRAD" in indices and len(parts) > indices["SRAD"] 
                               else 20.0),
...
def main():
    ...
    planting_date = "2016-10-08"  # From UFBA1601.SRX PDATE=16282 (day 282 of 2016)
    
    # Read weather data
    weather_dir = os.path.join("dssat-csm-data-develop", "Weather")
    wth_path = os.path.join(weather_dir, "UFBA1601.WTH")  # Use available weather file
    ...
    try:
        ...
        if 'DAP' in fort_df.columns:
            # Convert DAP to numeric, handling any non-numeric values
            dap_numeric = pd.to_numeric(fort_df['DAP'], errors='coerce')
            dap_valid = dap_numeric.dropna()
            if len(dap_valid) > 0:
                print(f"Days after planting range: {int(dap_valid.min())} to {int(dap_valid.max())}")
            else:
                print("Days after planting: No valid numeric DAP values found")
        print("Sample of first 5 rows:")
