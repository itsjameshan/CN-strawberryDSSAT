...
def parse_srx_file(path: str):
    # 从SRX文件中提取种植日期和气象站代码
    # 路径：dssat-csm-data-develop/Strawberry/UFBA1401.SRX
...
def read_wth_file(path: str) -> pd.DataFrame:
    # 读取天气数据
    # 路径：dssat-csm-data-develop/Weather/[气象站代码][年份].WTH
...
def read_fortran_output(exp_dir: str) -> pd.DataFrame:  # read DSSAT output files
    ...
    summary_path = os.path.join(exp_dir, "summary.csv")  # 首选
    if os.path.exists(summary_path):
        return pd.read_csv(summary_path)
    pg_path = os.path.join(exp_dir, "PlantGro.OUT")  # 备选
...
def run_python_model(wth_df: pd.DataFrame, planting_date: str):
    ...
    return model.simulate_growth(wth_df)  # 使用天气数据运行生长模拟
...
def main():  # orchestrate the comparison
    ...
    # Python模型需要手动读取和解析文件
    wth_df = read_wth_file(wth_path)  # 手动读取天气文件
    planting_date, wsta = parse_srx_file(args.srx)  # 手动解析SRX文件
