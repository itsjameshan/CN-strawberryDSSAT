...
def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("experiment", help="Path to .SRX experiment file")
    parser.add_argument("--dssat-dir", default="dssat-csm-os-develop", help="DSSAT root directory")
    ...
    dssat_exe = Path(args.dssat_dir) / "bin" / "dscsm048"
    dssat_exe = dssat_exe.resolve()

    # 可选：如果指定了输出目录，可以复制SRX到输出目录再运行
    run_dir = exp_dir
    if args.output_dir:
        ...
        # 复制实验文件到输出目录
        import shutil
        shutil.copy(exp_path, run_dir / exp_file)

    # 运行 DSSAT
    print(f"Running DSSAT in {run_dir} with {exp_file}")
    subprocess.run([str(dssat_exe), "A", exp_file], cwd=run_dir, check=True)
