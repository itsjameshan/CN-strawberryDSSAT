...
def parse_dssat_date(code) -> str:
    code = str(code)  # 保证是字符串
...
def read_wth_file(path: str) -> pd.DataFrame:
    with open(path) as f:
        lines = f.readlines()
    start = next(i for i, l in enumerate(lines) if l.startswith("@DATE"))
    header = lines[start].replace("@", "").split()
    # 只保留数据区
    data_lines = [l for l in lines[start+1:] if l.strip() and not l.startswith("*")]
    # 用pandas读入
    from io import StringIO
    df = pd.read_csv(StringIO("".join(data_lines)), sep='\\s+', names=header)
    # 解析日期
    df["date"] = df["DATE"].apply(parse_dssat_date)
    return df
...
def main():
    ...
    # 标准化天气数据列名，适配Python模型
    wth_df = wth_df.rename(columns={
        "TMAX": "tmax",
        "TMIN": "tmin",
