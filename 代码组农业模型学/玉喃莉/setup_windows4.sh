...
# Function to fix WSL network issues and configure Chinese mirrors
fix_wsl_network_and_mirrors() {
    print_status "Detecting and fixing WSL network issues..."
    
    # Check if we're in WSL
    if [[ -f "/proc/version" ]] && grep -qi "microsoft" /proc/version 2>/dev/null; then
        print_status "WSL environment detected. Configuring Chinese mirrors..."
        
...
        fi
        
        # Configure Chinese mirrors for Ubuntu
        print_status "Configuring Chinese Ubuntu mirrors..."
...
        print_status "Updating package lists with Chinese mirrors..."
        for i in {1..3}; do
            if sudo apt-get update --fix-missing; then
                print_success "Package lists updated successfully!"
                return 0
            else
                print_warning "Attempt $i failed. Trying alternative mirrors..."
                # Try Alibaba mirrors if Tsinghua fails
                sudo tee /etc/apt/sources.list > /dev/null << 'EOF'
# 阿里云镜像源
deb https://mirrors.aliyun.com/ubuntu/ noble main restricted universe multiverse
deb https://mirrors.aliyun.com/ubuntu/ noble-updates main restricted universe multiverse
...
        return 1
    else
        print_status "Not in WSL environment, skipping network fixes."
        return 0
    fi
}
...
                print_warning "Trying with --user flag (not in virtualenv)..."
                for mirror in "${MIRRORS[@]}"; do
...
                done
            else
                print_warning "In virtualenv, skipping --user install for $package."
            fi
        fi
        
        if [ "$installed" = false ]; then
