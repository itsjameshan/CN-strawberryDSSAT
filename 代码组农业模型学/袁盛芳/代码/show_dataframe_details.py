...
def main():
    ...
    wth_path = os.path.join(weather_dir, "WAPA9201_fixed.WTH")
    ...
    try:
        ...
        if 'DAP' in fort_df.columns:
            # 只保留能转成数字的 DAP，自动忽略非数字内容
            dap_numeric = pd.to_numeric(fort_df['DAP'], errors='coerce')
            print(f"Days after planting range: {dap_numeric.min()} to {dap_numeric.max()}")
        else:
            print("DAP column not found in DSSAT output")
        print("Sample of first 5 rows:")
    ...
    # 保存为CSV文件
    wth_df.to_csv("weather_data.csv", index=False, encoding="utf-8")
    py_df.to_csv("python_model_output.csv", index=False, encoding="utf-8")
    fort_df.to_csv("dssat_fortran_output.csv", index=False, encoding="utf-8")
    print("所有数据已保存为CSV文件！")
